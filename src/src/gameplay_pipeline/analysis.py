...
def detect_visual_cut_segments(
    ...
    for template_path in template_paths:
        ...
        if region_hint is not None:
            hinted_regions.add(region_hint)
        templates.append(
            (
                cleaned_label,
                template_scaled,
    ...
    if terminal_region_detector_enabled and hinted_regions:
        capture = cv2.VideoCapture(str(file_path))
        if capture.isOpened():
            ...
            region_start_times: dict[str, float | None] = {region: None for region in hinted_regions}
            sample_time = max(0.0, duration_seconds * terminal_region_start_fraction)
            last_time = sample_time
